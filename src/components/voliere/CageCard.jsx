...
export function CageCard({ cage, isSelected, onClick }) {
    const getColor = (statut) => {
        ...
        if (statut === 'couple') return 'bg-cage-single-soft ring-cage-single-border text-cage-single-border'
        if (statut === 'occupe') return 'bg-cage-couple-soft ring-cage-couple-border text-cage-couple-border'
    ...
    return (
        <button
            ...
            tabIndex={0}
            className={`group relative flex aspect-square flex-col items-center justify-between overflow-hidden rounded-xl ${getColor(cage.statut)} p-2 text-center shadow-sm ring-1 transition-all hover:-translate-y-0.5 hover:shadow-md ${isSelected ? "ring-2 ring-offset-2 ring-offset-background" : ""}`}
        >
