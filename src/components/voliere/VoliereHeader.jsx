 ...
 */
export function VoliereHeader({ filter, setFilter, query, setQuery, counts, showResultCount = false }) {
    const filters = [
        ...
        <div className="space-y-4 sm:space-y-6 mb-4 sm:mb-6">
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                ...
                <button
                    ...
                    className={`flex items-center gap-2 rounded-full px-3 py-1.5 text-xs ring-1 transition ${filter === "pigeon" ? "bg-cage-couple-soft ring-cage-couple-border" : "bg-card ring-border hover:bg-muted"}`}
                >
                    <span className="h-2.5 w-2.5 rounded-full bg-cage-couple-border" />
                    Pigeon seul ({counts.free})
                </button>
                <button
                    onClick={() => setFilter(filter === "couple" ? "all" : "couple")}
                    className={`flex items-center gap-2 rounded-full px-3 py-1.5 text-xs ring-1 transition ${filter === "couple" ? "bg-cage-single-soft ring-cage-single-border" : "bg-card ring-border hover:bg-muted"}`}
                >
