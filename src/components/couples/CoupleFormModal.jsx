...
/**
 * Modal de formulaire pour créer/modifier un couple
 */
export function CoupleFormModal({ isOpen, onClose, couple, onSubmit, isLoading }) {
    ...
    // Récupérer les pigeons disponibles OU tous les pigeons en mode édition
    ...
    // Filtrer les pigeons selon le mode
    const males = useMemo(() => {
        if (!pigeonsDisponibles) return []
        
        if (couple) {
            // Mode édition : afficher tous les mâles actifs
            const result = pigeonsDisponibles.filter(p => p.sexe === 'male' && p.statut === 'actif')
            console.log('Mâles disponibles (édition):', result)
            return result
        } else {
            // Mode création : uniquement les disponibles
            const result = pigeonsDisponibles.filter(p => p.sexe === 'male')
            console.log('Mâles disponibles (création):', result)
            return result
        }
    ...
    const femelles = useMemo(() => {
        ...
        if (couple) {
            // Mode édition : afficher toutes les femelles actives
            const result = pigeonsDisponibles.filter(p => p.sexe === 'femelle' && p.statut === 'actif')
            console.log('Femelles disponibles (édition):', result)
            return result
        } else {
            // Mode création : uniquement les disponibles
            const result = pigeonsDisponibles.filter(p => p.sexe === 'femelle')
            console.log('Femelles disponibles (création):', result)
            return result
        }
    }, [pigeonsDisponibles, couple])
    ...
    useEffect(() => {
        if (couple) {
            // Mode édition : pré-remplir le formulaire avec les IDs des pigeons
            console.log('Couple à éditer:', couple)
            const maleId = couple.male_id || couple.male?.id || ''
            const femelleId = couple.femelle_id || couple.femelle?.id || ''
            console.log('Male ID:', maleId, 'Femelle ID:', femelleId)
            
            setFormData({
                male_id: String(maleId), // Convertir en string pour le select
    ...
    const validate = () => {
        ...
        if (!formData.male_id) {
            newErrors.male_id = 'Le mâle est requis'
        }

        if (!formData.femelle_id) {
            newErrors.femelle_id = 'La femelle est requise'
        }

        if (!formData.date_formation) {
            newErrors.date_formation = 'La date de formation est requise'
        }
        ...
        >
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-6">
                ...
                <div>
                    ...
                    {errors.male_id && (
                        <p className="text-destructive text-xs mt-1">{errors.male_id}</p>
                    )}
                    {males.length === 0 && (
                ...
                <div>
                    ...
                    {errors.femelle_id && (
                        <p className="text-destructive text-xs mt-1">{errors.femelle_id}</p>
                    )}
                    {femelles.length === 0 && (
